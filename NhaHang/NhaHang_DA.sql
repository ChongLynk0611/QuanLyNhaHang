...
create table PhieuXuatKho(
	id_PhieuXuatKho int primary key identity(1,1),
	id_NhanVien varchar(20) references NhanVien(id_NhanVien),
	NgayNhap date default getdate()
);
create table ChiTietXuatKho(
	id_PhieuXuatKho int references PhieuXuatKho(id_PhieuXuatKho),
	id_NguyenLieu int references NguyenLieu(id_NguyenLieu),
	SoLuong int not null,
	DonVi nvarchar(50)
);
...
CREATE PROC thongke_nguyenlieunam( @nam int)
AS
BEGIN
SELECT PhieuNhapKho.id_PhieuNhapKho, ChiTietNhapKho.id_NguyenLieu, NguyenLieu.TenNL, ChiTietNhapKho.SoLuong, ChiTietNhapKho.Gia--nguyen lieu thua so luong
FROM PhieuNhapKho, ChiTietNhapKho, NguyenLieu
...
------------------------------ PROC ĐỨC  ANH --------------------------------
alter table NguyenLieu add DonVi varchar(50)
alter table ChiTietNhapKho add DonVi varchar(50)
alter table ChiTietXuatKho add DonVi varchar(50)
...
create proc LSXuatKho
as begin
	select CTXK.id_PhieuXuatKho,PXK.id_NhanVien,NV.HoTen,PXK.NgayNhap
	from ChiTietXuatKho as CTXK, PhieuXuatKho as PXK, NhanVien as NV 
	where PXK.id_PhieuXuatKho = CTXK.id_PhieuXuatKho and NV.id_NhanVien=PXK.id_NhanVien
	group by CTXK.id_PhieuXuatKho,PXK.id_NhanVien,NV.HoTen,PXK.NgayNhap
end
go
...
create proc add_NL
(@ten_NL nvarchar(50), @SL int, @DonVi nvarchar(50))
as begin
	insert into NguyenLieu values(@ten_NL,@SL,@DonVi)
end
go

create proc update_NL
(@ma_NL int, @ten_NL nvarchar(50), @DonVi nvarchar(50))
as begin
	update NguyenLieu set TenNL = @ten_NL,DonVi = @DonVi where id_NguyenLieu = @ma_NL
end
go
...
(@ma_PNK int, @ma_NL int, @SL int, @Gia int,@DonVi varchar(50))
as begin
	declare @count int
	select @count = count(*) from ChiTietNhapKho where id_NguyenLieu = @ma_NL and id_PhieuNhapKho = @ma_PNK 
	if @count = 0
		insert into ChiTietNhapKho values (@ma_PNK,@ma_NL,@SL,@Gia,@DonVi)
	else
...
create proc add_CTXK
(@ma_PXK int, @ma_NL int, @SL int,@DonVi varchar(50))
as begin
	declare @count int
	select @count = count(*) from ChiTietXuatKho where id_NguyenLieu = @ma_NL and id_PhieuXuatKho = @ma_PXK 
	if @count = 0
		insert into ChiTietXuatKho values (@ma_PXK,@ma_NL,@SL,@DonVi)
	else
		update ChiTietXuatKho set SoLuong = SoLuong + @SL where id_NguyenLieu = @ma_NL and id_PhieuXuatKho = @ma_PXK
end
go
...
(@ma_NV varchar(20), @ngayNhap date)
as begin
	insert into PhieuXuatKho(id_NhanVien,NgayNhap) values(@ma_NV,@ngayNhap)
end
go
...
@ma_PNK int
as begin
	select CTNK.id_NguyenLieu,NL.TenNL,NL.DonVi,CTNK.SoLuong,Gia from ChiTietNhapKho as CTNK,NguyenLieu as NL 
...
@ma_PXK int
as begin
	select CTXK.id_NguyenLieu,NL.TenNL,NL.DonVi,CTXK.SoLuong from ChiTietXuatKho as CTXK,NguyenLieu as NL 
	where id_PhieuXuatKho = @ma_PXK and CTXK.id_NguyenLieu=NL.id_NguyenLieu
end
go
